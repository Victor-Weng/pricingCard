...
def get_prompt(innovation_ratio, keys, last_prompt):
    ...
    '''
    PROMPT = f"I need you to generate 4 things for me. 1. A one-sentence prompt for AI generative HOME ARCHITECTURE art, 2. unique key terms \
    ...
    Furthermore, he order that you will see the upcoming keys \
    DOES NOT MATTER AT ALL, you must look at the WEIGHTS indicated by \" weight \" to understand that the higher weights corresponds to better key terms. \
    MAKE SURE YOUR PROMPT IS COMPLETELY DIFFERENT from your last prompt, {last_prompt}. Remember this as GENERATED_PROMPT\
    ...
    Weight count means a more successful and benficial Key Term. Remember, you MUST be creative and add YOUR OWN key terms. Use about {innovation_ratio}% new key words. Key words should be ONE single word. \
    Remember ALL the keys used (old and new) as KEYS_USED with each separated by a comma. Ex. architecture, brutalist, minimalist\
    ...
    \"prompt\" : \"Cozy cottage in the forest with sunlight pouring in, lush greenery, and a small trail.\", \"key\" : [\"modern\", \"wooden\", \"library\", \"Zaha-Hadid\"], \
    \"caption\" : \"Would you live here? \", \"hashtag\" : \"#illustrations #architecture #cozy #cozyhomes \
    #forest #cottage #forestvibe #summer \" This example is just for demonstration. \
    You should aim to be more creative, and aim to include keys such as SIMPLE, focused, clean-lighting, pleasing, photo-realistic, realistic \
    Remember, you are generating ONE SINGLE HOME, not city scapes. Make it obvious that only one photo should be generated by \
    Starting your prompt like \" a single zoomed out photograph of... \" Keep your prompt character limit to below 100, make sure\
    that the prompt describes something SIMPLE, and to not be too over descriptive.\
    Here is the list of Key Terms to base your generation upon : {keys}"

    print("sent to EdgeGPT")
